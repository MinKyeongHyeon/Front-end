...
function App() {
  ...
  const checkThreeThree = (testBoard, row, col, player) => {
    ...
    for (let [dx, dy] of directions) {
      ...
      // 활삼 조건: 3개 연속이면서 양쪽이 모두 열려있음
      if (lineInfo.count === 3 && !lineInfo.leftBlocked && !lineInfo.rightBlocked) {
        openThreeCount++;
      }
    }
  ...
  const checkForbiddenMove = (testBoard, row, col, player) => {
    ...
    if (checkThreeThree(testBoard, row, col, player)) {
      return { isForbidden: true, message: '3-3 금수입니다!' };
    }
    
    if (checkFourFour(testBoard, row, col, player)) {
      return { isForbidden: true, message: '4-4 금수입니다!' };
    }
